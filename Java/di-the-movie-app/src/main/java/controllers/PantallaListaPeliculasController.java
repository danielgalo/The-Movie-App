...
/**
 * Pantalla donde se muestra las películas insertadas por el usuario, además
 * permite buscarlas por título e incluso borrarlas.
 */
public class PantallaListaPeliculasController {
	...
	void initialize() {
		...
		if (buscar && !tituloBuscar.isBlank()) {
			// Añade a la lista de películas solo las que incluyan los caracteres de la
			// búsqueda
			listPeliculas = buscadorPeliculas.searchMovieByTitle(tituloBuscar,
		...
		/**
		 * Nota: Esta variable determina la posición vertical de la primera celda de
		 * peliculas a mostrar, de aquí en adelante esta variable se aumenta para la
		 * creación del resto de celdas.
		 */
		int posY = 220;
		...
		for (Pelicula pelicula : listPeliculas) {
			...
			/**
			 * Al pulsar en el título de la película, se muestra una ventana con datos
			 * extendidos de la película
			 * 
			...
			celdaPelicula.getLabelTituloPulsable().setOnMousePressed(new EventHandler<MouseEvent>() {
				...
				public void handle(MouseEvent event) {
					...
					NavegacionPantallas navegacionPantallas = new NavegacionPantallas("Información pelicula",
							Constantes.PANTALLA_INFORMACION_PELICULA, Constantes.CSS_INFORMACION_PELICULA);
		...
		// Añade todas las celdas de la lista de celdas a la pantalla de lista de
		// películas
		mainPane.getChildren().addAll(listaCeldasPeliculas);
		// Ajusta la altura de la pantalla de lista de películas según la cantidad de
		// celdas visibles
		mainPane.setPrefHeight(posY + 50);
		/**
		 * Nota: Para evitar que la pantalla de lista sea muy pequeña en caso haya muy
		 * pocas celdas, si su altura es menor que la predeterminada, vuelve a tener ese
		 * valor
		 */
	...
	void btnVolverPressed(MouseEvent event) {
		NavegacionPantallas navegacion = new NavegacionPantallas("Pantalla Principal", Constantes.PANTALLA_PRINCIPAL,
				Constantes.CSS_PANTALLA_PRINCIPAL);
